...
class PlanController extends Controller
{
    ...
    public function save(PlanSave $request)
    {
        ...
        if ($request->input('id')) {
            ...
            if (!$plan) {
                abort(500, __('This subscription does not exist'));
            }
            DB::beginTransaction();
    ...
    public function drop(Request $request)
    {
        if (Order::where('plan_id', $request->input('id'))->first()) {
            abort(500, __('Orders exist under this subscription, it cannot be deleted'));
        }
        if (User::where('plan_id', $request->input('id'))->first()) {
            abort(500, __('Users exist under this subscription, it cannot be deleted'));
        }
        if ($request->input('id')) {
            $plan = Plan::find($request->input('id'));
            if (!$plan) {
