...
/**
 * Paytaro popup QR payment interface (V2Board) -- no hosted checkout mode
 * Generated by the Paytaro one-click script install-qr.sh. Can coexist with the hosted-checkout Paytaro.php.
 * In the admin panel under "Payment config", select PaytaroQR when adding; add one entry per Paytaro payment method (Alipay / USDT-TRC20 ...) and fill in the corresponding payment method UUID.
 * Docs: https://v3.paytaro.com/#/docs/install-script   Support: https://t.me/paytaro
 */
...
class PaytaroQR
{
    ...
    public const PAGE_PATH = '/paytaro-qr/pay.php'; // Popup page under the panel domain (used for crypto)
    ...
    public function pay($order)
    {
        ...
        $payload = [
            ...
            'order_amount' => round(((int) $order['total_amount']) / 100, 2), // V2Board counts in cents
        ...
        // V2Board: type 0 = show QR code in panel dialog, 1 = redirect to data
        if ($isAlipay) {
            if ($this->isMobile()) {
                return ['type' => 1, 'data' => !empty($payment['mobile_url']) ? $payment['mobile_url'] : $payment['data']];
            }
            if ($linkType === 'pc') {
                return ['type' => 1, 'data' => $payment['data']]; // Desktop website payment: go straight to the Alipay PC checkout
            }

            return ['type' => 0, 'data' => $payment['data']]; // Mobile website payment link: render the code on PC, scan it with Alipay
        }

        // Crypto: the panel's native dialog cannot fit the amount and copy buttons, so jump to the dialog page under this site's domain
