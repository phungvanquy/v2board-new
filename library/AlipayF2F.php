...
class AlipayF2F {
    ...
    public function send()
    {
        ...
        if (!isset($response[$resKey])) throw new \Exception('Request to Alipay failed');
    ...
    {
        if (!$query) {
            throw new \Exception('Parameter building error');
        }
        // Sort the parameters
        ksort($query);
    ...
    private function buildSign(string $signData): string
    {
        ...
        $p_key = array();
        // If the private key is a single line
        ...
        // The encrypted content usually contains special characters and needs encoding conversion
