...
<div class="wrap">
    <div class="card">
        ...
        <p class="muted">{{ __('Control which destinations bypass the VPN (go DIRECT) for all clients. Changes apply on the next subscription update — no server restart needed.') }}</p>
        <p class="muted"><a href="{{ url('/' . $secure_path) }}">&larr; {{ __('Back to admin') }}</a></p>
    </div>

    <div class="card">
        <h2>{{ __('Russia — bypass VPN (DIRECT)') }}</h2>
        <p class="muted">{{ __('When enabled, Russian sites (.ru / .su / .рф / moscow / tatar, vk/yandex/kaspersky) and RU IPs are routed DIRECT for every subscription format (Clash, Stash, Surge, Surfboard, sing-box/Hiddify).') }}</p>
        ...
        <div class="form-row">
            ...
            <span class="hint">{{ __('Turn off to proxy RU traffic like any other destination.') }}</span>
        </div>
        ...
        <div class="form-row">
            ...
            <span class="hint">{{ __('One per line. Each value becomes DOMAIN-SUFFIX,…,DIRECT. Base list (ru/su/xn--p1ai/moscow/tatar + vk/yandex/kaspersky) is always included when enabled.') }}</span>
        </div>
...
function renderPreview() {
    const extra = (document.getElementById('ruDomains').value || '').split(/[\r\n,;]+/).map(s=>s.trim().toLowerCase().replace(/^\.+/,'')).filter(s=>s && /^[a-z0-9.\-]+$/.test(s));
...
function doSave() {
    ...
    api('/subscribe-rules/save', {
        ...
        showMsg('saveMsg', 'Saved. Clients will see it on next subscription update.', 'success');
        showMsg('alertBox', '', '');
        renderPreview();
    }).catch(e=>{
